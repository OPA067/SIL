...
class ActivityNetDataset(Dataset):
    def __init__(
        ...
        video_id_path_dict["test"] = os.path.join(self.data_path, "val_ids.json")

        video_json_path_dict = {}
        video_json_path_dict["train"] = os.path.join(self.data_path, "train.json")
        video_json_path_dict["test"] = os.path.join(self.data_path, "val_1.json")

        cap_json_path = os.path.join(self.data_path, "ActivityNet_VILA_F6.json")

        pseudo_video_id_list, video_id_list = self._get_video_id_single(video_id_path_dict[self.subset])
        pseudo_caption_dict = self._get_captions_single(video_json_path_dict[self.subset], cap_json_path)
    ...
    def _get_captions_single(self, path, cap_json_path):
        pseudo_caption_dict = {}
        with open(path, 'r') as f:
            json_data = json.load(f)
    ...
    def __getitem__(self, feature_idx):
        ...
        pairs_text, pairs_mask, pairs_segment, pairs_text_list, pairs_mask_list, pairs_segment_list, starts, ends = self._get_text(pseudo_video_id, sub_id)
        video, video_mask = self._get_rawvideo(self.video_id_list[idx], starts, ends)
        return pairs_text, pairs_mask, pairs_text_list, pairs_mask_list, video, video_mask, feature_idx, hash(pseudo_video_id)
