...
def get_args(description='Text-Video Retrieval.'):
    ...
    parser.add_argument("--output_dir", type=str, default=None, required=True, help="The output directory where the model predictions and checkpoints will be written.")
    ...
    parser.add_argument('--beta', type=float, default=0.1, help='hyper-parameters beta')
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, max_steps):
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        text, text_mask, text_list, text_mask_list, video, video_mask, idx, _ = batch
        if n_gpu == 1:
            text, text_mask, video, video_mask, idx = [x.to(device=device, non_blocking=True) for x in [text, text_mask, video, video_mask, idx]]
            text_list = [t.to(device=device, non_blocking=True) for t in text_list]
            text_mask_list = [m.to(device=device, non_blocking=True) for m in text_mask_list]

        loss = model(text, text_mask, text_list, text_mask_list, video, video_mask, idx, global_step)
...
def _run_on_single_gpu(model, batch_s_feat, batch_c_feat, batch_f_feat, split_batch=32):

    sim_matrix = []

    batch_s_feat = torch.split(batch_s_feat, split_batch)
    batch_c_feat = torch.split(batch_c_feat, split_batch)
    ...
    with torch.no_grad():
        for idx1, s_feat in tqdm(enumerate(batch_s_feat)):
            ...
            for idx2, (c_feat, f_feat) in enumerate(zip(batch_c_feat, batch_f_feat)):
                logits = model.get_similarity_logits(s_feat, c_feat, f_feat)
...
def eval_epoch(args, model, test_dataloader, device):
    ...
    ids, batch_s_feat, batch_c_feat, batch_f_feat = [], [], [], []

    with torch.no_grad():
        tic = time.time()
        for batch in tqdm(test_dataloader):
            text, text_mask, text_list, text_mask_list, video, video_mask, inds, index = batch
            text, text_mask, video, video_mask, inds, index = [x.to(device) for x in [text, text_mask, video, video_mask, inds, index]]
            text_list = [x.to(device) for x in text_list]
            ...
            batch_c_feat.append(c_feat)
            batch_f_feat.append(f_feat)

        ids = allgather(torch.cat(ids, dim=0), args).squeeze()
        batch_s_feat = allgather(torch.cat(batch_s_feat, dim=0), args)
        batch_c_feat = allgather(torch.cat(batch_c_feat, dim=0), args)
        batch_f_feat = allgather(torch.cat(batch_f_feat, dim=0), args)

    toc1 = time.time()

    with torch.no_grad():
        sim_matrix = _run_on_single_gpu(model, batch_s_feat, batch_c_feat, batch_f_feat, args.split_batch)
    ...
    if multi_sentence_:
        ...
        logger.info("Text-to-Video: R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - R@Sum: {:.1f} - MdR: {:.1f} - MnR: {:.1f}".
        ...
        logger.info("Video-to-Text: R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - R@Sum: {:.1f} - MdR: {:.1f} - MnR: {:.1f}".
